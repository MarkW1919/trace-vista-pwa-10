...
export const ConsentWarning = ({ variant = 'default' }: ConsentWarningProps) => {
  if (variant === 'prominent') {
    return (
      <div className="mb-6 rounded-lg border-l-4 border-destructive bg-destructive/5 p-6">
        <div className="flex items-start space-x-4">
          <div className="flex-shrink-0">
            <AlertTriangle className="h-6 w-6 text-destructive" />
          </div>
          <div className="flex-1">
            <h3 className="flex items-center text-lg font-semibold text-destructive-foreground">
              <Shield className="mr-2 h-5 w-5" />
              Consent Required - Real Data Only
            </h3>
            <div className="mt-3 space-y-3 text-sm text-destructive-foreground/90">
              <p className="font-bold">
                ⚠️ By using this tool, you confirm the subject has provided explicit consent for this search.
              </p>
              <div className="grid gap-2 sm:grid-cols-2">
                <div className="flex items-start space-x-2">
                  <CheckCircle className="mt-0.5 h-4 w-4 flex-shrink-0 text-destructive" />
